...
import {T_MedicinesFilters} from "modules/types.ts";
import MedicinesTable from "components/MedicinesTable/MedicinesTable.tsx";

const MedicinesPage = () => {

    const medicines = useAppSelector((state) => state.medicines.medicines)

    const {is_authenticated, is_superuser} = useAppSelector((state) => state.user)

    const filters = useAppSelector<T_MedicinesFilters>((state) => state.medicines.filters)
    ...
    useEffect(() => {
        const intervalId = setInterval(handleFetchMedicines, 2000)
        return () => clearInterval(intervalId)
    }, [filters]);

    const handleFetchMedicines = () => dispatch(fetchMedicines())

    const applyFilters = async (e) => {
        e.preventDefault()

        const filters:T_MedicinesFilters = {
            status: status,
    ...
    return (
        <Container>
            ...
            {medicines.length ? <MedicinesTable medicines={medicines} /> : <h3 className="text-center mt-5">Лекарства не найдены</h3>}
        </Container>
    )
};
