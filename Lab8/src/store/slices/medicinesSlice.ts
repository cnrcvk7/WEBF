...
export const fetchMedicines = createAsyncThunk<T_Medicine[], object, AsyncThunkConfig>(
    ...
    async function(_, thunkAPI) {
        ...
        return response.data.filter(medicine => medicine.owner.includes(state.medicines.filters.owner))
    }
)
...
export const acceptMedicine = createAsyncThunk<void, string, AsyncThunkConfig>(
    "medicines/accept_medicine",
    async function(medicine_id,{dispatch}) {
        await api.medicines.medicinesUpdateStatusAdminUpdate(medicine_id, {status: 3})
        await dispatch(fetchMedicines)
    }
)

export const rejectMedicine = createAsyncThunk<void, string, AsyncThunkConfig>(
    "medicines/accept_medicine",
    async function(medicine_id,{dispatch}) {
        await api.medicines.medicinesUpdateStatusAdminUpdate(medicine_id, {status: 4})
        await dispatch(fetchMedicines)
    }
)
